...
def documents_start(update, context):
    context.user_data['dialog'] = {'service': update.message.text}
    reply_keyboard = [['Замена паспорта РФ'], ['Замена загранпаспорта']]
...
def documents_contacts(update, context):
...
def documents_comment(update, context):
...
def documents_ending(update, context):
